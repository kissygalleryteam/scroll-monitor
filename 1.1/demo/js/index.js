var isMini = !KISSY.stamp,
    modules = ['node','io','base','gallery/scroll-monitor/1.1/'];
    
if (isMini) {
    modules = modules.concat(['juicer', 'lang']);
} else {
    modules = modules.concat(['gallery/juicer/0.6.5/']);
}


S.use(modules.join(','), function (S, Node, Ajax, Base, ScrollMonitor, Juicer, Lang) {
    
    var DribbbleLoader = Base.extend({
